...
stand_type <- dbGetQuery(con,'select a.feature_id,a.stand_type,b.stand_label from integration_final a left join stand_types b using(stand_type)')


si_not_in_species <- si_data %>% subset(!feature_id %in% species$feature_id)
species_not_in_si <- species %>% subset(!feature_id %in% si_data$feature_id)

si_not_in_species <- left_join(si_not_in_species,stand_type)

species_not_in_si <- left_join(species_not_in_si,stand_type)
...
# why are these so different in the row counts.
...
# run the base conversions on BEC in the final missing site index values
#avg_BEC_data <- base_si_conversion(avg_BEC_data)

# run the bec conversions again
#fix_si <- bec_conversions(fix_si)
...
c1 <- count(fix_si,missing,bec_zone,bec_subzone) %>% arrange(desc(n))
kable(c1,format='markdown')
...
c2$ord <- case_when(
  c2$src == 'pspl' ~ 1,
  c2$src == 'spruce_conv' ~ 2,
  c2$src == 'base1' ~ 3,
  c2$src == 'bec_convert' ~ 4,
  c2$src == 'base2' ~ 5,
  TRUE ~ 99
)
...
c2$pct <- case_when(
  ...
  c2$src == 'spruce_conv' ~ c2$n/sum(c2$n) * 100,
  c2$src == 'base1' ~ c2$n/sum(c2$n) * 100,
  c2$src == 'bec_convert' ~ c2$n/sum(c2$n) * 100,
  c2$src == 'base2' ~ c2$n/sum(c2$n) * 100,
...
| spruce_conv | Spruce conversions form SX to Sw, Se, Ss |
| base1 | SIndex site index conversions |
| bec_convert | Site index from BEC mean values |
| base2 | SIndex conversions run a second time | 
