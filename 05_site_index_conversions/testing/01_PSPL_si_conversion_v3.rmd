...
```{r  si_convert_op,echo=TRUE,message=TRUE,warning=TRUE}
...
species <- dbGetQuery(con,"select opening_id,array_to_string(species,',') as species from species_list_op")
...
avg_data <- call_si_conversion(avg_data)

avg_data <- assign_missing(avg_data)
avg_data <- avg_data %>% subset(missing != '') 
...
```{r update_fid_using_bec,eval=FALSE,echo=TRUE,message=TRUE,warning=TRUE}
